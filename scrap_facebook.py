...
# Asegúrate de que estas funciones existan en tu archivo utils.py
from utils import limpiar_texto, guardar_comentarios_csv, guardar_procesados_csv
from consulta_llama import analizar_comentarios

SESSION_FILE = "session_facebook.json"
...
async def iniciar_scrapping(tema, n_posts, n_comentarios):
    async with async_playwright() as p:
        ...
        print(f"[Facebook] Proceso finalizado. {len(comentarios)} comentarios guardados.")
        await browser.close()

if __name__ == "__main__":
    asyncio.run(iniciar_scrapping("venezuela", 5, 10))
